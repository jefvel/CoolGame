...
import kek.audio.AudioGenerator;
...
class AudioApp {
	...
	var playing = false;
	function keyDown(key:kha.Key, id:String){
		
	}
	
	function keyUp(key:kha.Key, id:String) {
	...
	function initSound() {
		...
		for(i in 0...echoBuf.length) echoBuf[i] = 0.0;
		//Assets.initSound(Assets.sounds.testName, function(s:kha.Sound) {
		//	s.uncompress(function(){
				position = 0;//-samplesPerSecond * 10;
	//			sound = s;
	//			trace(s.uncompressedData.length);
				playing = true;
	...
	function stream(samples:Int, buf:kha.audio2.Buffer) {
		...
		//Vector.blit(sound.uncompressedData, position, buf.data, buf.writeLocation, samples);
		for(i in 0...samples) {
